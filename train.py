...
def build_batches(sequences, tok, block_size, device):
    """Each training example = one user history minus its held-out last item,
    flattened to tokens, right-padded to block_size. Returns (X, Y) tensors for
    next-token prediction."""
    X, Y = [], []
    for seq in sequences:
        if len(seq) < 2:
            continue
        toks = tok.encode_sequence(seq[:-1])  # drop the leave-one-out target
...
def main():
    ...
    D = tok.D
    print(f"[train] {len(sequences):,} users | Semantic ID length D={D} | "
          f"vocab={tok.vocab_size}")

    # block_size covers a full flattened history plus BOS, leaving room for the
    # D extra tokens we generate at eval time (history-minus-last + new item).
    block_size = cfg.max_seq_len * D + 1
    X, Y = build_batches(sequences, tok, block_size, device)
    print(f"[train] {X.shape[0]:,} training sequences | block_size={block_size}")
    ...
    print(f"[train] RecGPT {count_params(model):,} params")
    opt = model.configure_optimizers(cfg.weight_decay, cfg.gpt_lr, device)

    n = X.shape[0]
    for epoch in range(cfg.gpt_epochs):
        model.train()
        ...
        if epoch % 5 == 0 or epoch == cfg.gpt_epochs - 1:
            print(f"  epoch {epoch:3d} | loss {total / n:.4f}")

    torch.save({"model": model.state_dict(), "gptcfg": gptcfg.__dict__}, OUT / "recgpt.pt")
